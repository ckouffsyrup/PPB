...
items=items.map(i=>({
  variants:[],deal_qty:0,deal_price:0,out_of_stock_behavior:"show",inventory_mode:"stocked",filament_usage:[],
...
function isMadeToOrder(i){return i?.inventory_mode==="made_to_order"||i?.out_of_stock_behavior==="made_to_order"}
...
function normalizePublicPrint(p){
  return {id:p.id,name:p.name||"Print",category:p.category||"",price:Number(p.price||0),hours:p.hours??"",notes:p.notes||"",photo_url:p.photo_url||"",favorite:!!p.favorite,variants:Array.isArray(p.variants)?p.variants:[],deal_qty:Number(p.deal_qty||0),deal_price:Number(p.deal_price||0),out_of_stock_behavior:p.out_of_stock_behavior||"show",inventory_mode:p.inventory_mode||(p.out_of_stock_behavior==="made_to_order"?"made_to_order":"stocked"),made_qty:Number(p.made_qty||0),sold_qty:Number(p.sold_qty||0),multicolor_capable:!!p.multicolor_capable,multicolor_max_colors:Math.max(2,Number(p.multicolor_max_colors||2)),multicolor_price_mode:p.multicolor_price_mode==="per_extra"?"per_extra":"flat",multicolor_surcharge:Number(p.multicolor_surcharge||0),filament_usage:[],extra_cost:0,model_source:"",created_at:p.created_at||"",updated_at:p.updated_at||""}
}
...
function renderShop(){
  ...
  let list=items.filter(i=>{
    const stock=itemStock(i),madeToOrder=isMadeToOrder(i);
    if(!madeToOrder&&stock<=0&&i.out_of_stock_behavior==="hide")return false;
...
function orderVariantForReservation(o,item){
  if(!item)return null;
  if(o?.variant_id)return (item.variants||[]).find(v=>String(v.id)===String(o.variant_id))||null;
  const match=String(o?.notes||"").match(/^Version:\s*(.+)$/mi);
  if(match&&match[1].trim().toLowerCase()!=="standard")return (item.variants||[]).find(v=>String(v.name||"").trim().toLowerCase()===match[1].trim().toLowerCase())||null;
  return null;
}
...
function orderCustomerHistory(email,currentId=""){
  ...
  const matches=orders.filter(o=>String(o.customer_email||"").trim().toLowerCase()===key&&o.id!==currentId);if(!matches.length)return null;
  const paid=matches.reduce((sum,o)=>sum+orderCollectedAmount(o),0),completed=matches.filter(o=>o.status==="Completed").length,last=[...matches].sort((a,b)=>Date.parse(b.created_at||0)-Date.parse(a.created_at||0))[0];
  return {matches,paid,completed,last};
}
function renderCustomerHistory(){
  const card=$("customerHistoryCard");if(!card)return;const h=orderCustomerHistory($("orderCustomerEmail")?.value,editingOrderId);card.classList.toggle("hidden",!h);if(!h)return;
  $("customerHistoryName").textContent=(h.last?.customer||$("orderCustomer")?.value||"Customer")+" has ordered before";$("customerHistoryOrders").textContent=`${h.matches.length} previous order${h.matches.length===1?"":"s"}`;$("customerHistorySpent").textContent=money(h.paid);$("customerHistoryCompleted").textContent=h.completed;$("customerHistoryLast").textContent=h.last?.created_at?new Date(h.last.created_at).toLocaleDateString():"—";
}
function orderCardHTML(o,{board=false}={}){
  ...
  return `<article class="order-card order-card-v3 ${board?"board-order-card":""}" onclick="openOrder('${o.id}')"><div class="order-main"><div class="order-card-topline"><span class="status ${orderStatusClass(o.status)}">${safe(o.status)}</span>${o.order_number?`<span class="order-number">${safe(o.order_number)}</span>`:""}</div><h4>${safe(o.item||"Custom order")}</h4><p class="order-customer-line">${safe(o.customer||"Customer")}${history?` <span class="returning-customer">↻ Returning</span>`:""} · Qty ${o.quantity||1}</p>${o.due_date?`<p class="order-due">Due ${safe(o.due_date)}</p>`:""}${quoteState}${notes&&!board?`<p class="muted order-notes-preview">${safe(notes)}</p>`:""}</div><div class="order-side"><strong>${money(o.quoted_price)}</strong><span class="payment-mini">${safe(orderPaymentLabel(o))}</span>${next?`<button class="primary order-advance-btn" type="button" onclick="event.stopPropagation();advanceOrderStatus('${o.id}',event)">${safe(next[1])}</button>`:""}${!board&&o.status==="Quoted"?`<button class="secondary order-advance-btn" type="button" onclick="event.stopPropagation();sendQuoteEmailForOrder('${o.id}',{force:true}).then(()=>pullOrdersCloud())">Resend quote</button>`:""}${!board&&o.status==="Ready"?`<button class="secondary order-advance-btn" type="button" onclick="event.stopPropagation();sendReadyEmailForOrder('${o.id}',{force:true}).then(()=>pullOrdersCloud())">Resend ready email</button>`:""}</div></article>`;
}
function renderOrders(){
  ...
  const q=String($("orderSearch")?.value||"").trim().toLowerCase(),sorted=[...orders].filter(o=>orderSearchMatches(o,q)).sort((a,b)=>String(a.due_date||"9999").localeCompare(String(b.due_date||"9999"))||String(b.created_at||"").localeCompare(String(a.created_at||""))),list=sorted.filter(o=>!orderStatusFilter||o.status===orderStatusFilter);
  $("orderList").innerHTML=list.map(o=>orderCardHTML(o)).join("");
  const boardStatuses=["Requested","Quoted","Accepted","Approved","Printing","Ready"];$("orderBoard").innerHTML=boardStatuses.map(status=>{const group=sorted.filter(o=>o.status===status);return `<section class="order-board-column"><div class="order-board-heading"><span>${safe(status)}</span><strong>${group.length}</strong></div><div class="order-board-cards">${group.map(o=>orderCardHTML(o,{board:true})).join("")||`<div class="board-empty">Nothing here</div>`}</div></section>`}).join("");
  $("orderList").classList.toggle("hidden",ordersDisplayMode==="board");$("orderBoard").classList.toggle("hidden",ordersDisplayMode!=="board");$("orderFilter").classList.toggle("hidden",ordersDisplayMode==="board");$("orderEmpty").classList.toggle("hidden",ordersDisplayMode==="board"?sorted.some(o=>boardStatuses.includes(o.status)):!!list.length);
}
function setOrdersDisplayMode(mode){ordersDisplayMode=mode==="board"?"board":"list";$("ordersListViewBtn")?.classList.toggle("active",ordersDisplayMode==="list");$("ordersBoardViewBtn")?.classList.toggle("active",ordersDisplayMode==="board");try{localStorage.setItem("printbook_orders_view",ordersDisplayMode)}catch{}renderOrders()}
...
function populateOrderVariants(selected=""){
  const item=items.find(i=>i.id===$("orderPrint")?.value),sel=$("orderVariant");if(!sel)return;
  sel.innerHTML=`<option value="">Standard / no variant</option>`+(item?.variants||[]).map(v=>`<option value="${safe(v.id)}">${safe(v.name)}${isMadeToOrder(item)?" · made to order":` · ${Number(v.stock||0)} in stock`}</option>`).join("");
  if(selected&&(item?.variants||[]).some(v=>String(v.id)===String(selected)))sel.value=selected;
}
...
async function savePrint(){if(!requireOnlineAdminSave())return;
  ...
  try{
    ...
    let item={
      ...
      inventory_mode:$("inventoryModeInput").value==="made_to_order"?"made_to_order":"stocked",
      out_of_stock_behavior:$("inventoryModeInput").value==="made_to_order"?"made_to_order":$("outOfStockInput").value,
...
function splitOrderPaymentInstructions(notes){
  ...
  const cleanNotes=lines.filter(line=>!line.trim().toLowerCase().startsWith(PAYMENT_NOTE_PREFIX.toLowerCase())).join("\n").trim();
  return {instructions,cleanNotes};
}
function joinOrderPaymentInstructions(notes,instructions){
...
async function createOrderPaymentLink(){
  ...
  const quote=Math.max(0,Number($("orderPrice").value||0)),paid=Math.max(0,Number($("orderPaymentAmount").value||0)),deposit=Math.max(0,Number($("orderDepositAmount").value||0));
  if(quote<=paid)return toast("This order is already fully paid");
  const btn=$("createPaymentLinkBtn"),old=btn.textContent;btn.disabled=true;btn.dataset.busy="1";btn.textContent="Creating Stripe link…";
  try{
    const {data:{session}}=await supabaseClient.auth.getSession();if(!session?.access_token)throw new Error("Sign in again first.");
    const base=String(settings.supabaseUrl||"").replace(/\/$/,"");if(!base)throw new Error("Supabase URL is missing.");
    const kind=deposit>paid?"deposit":"balance";
    const r=await fetch(`${base}/functions/v1/stripe-checkout`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${session.access_token}`,...(settings.supabaseKey?{apikey:settings.supabaseKey}:{})},body:JSON.stringify({order_id:editingOrderId,kind,return_url:location.origin+location.pathname})});
    const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||"Could not create payment link.");
    $("orderPaymentLink").value=d.url||"";$("orderPaymentMethod").value="Stripe";const o=orders.find(x=>x.id===editingOrderId);if(o){o.payment_link=d.url;o.payment_provider="stripe";o.stripe_checkout_session_id=d.session_id;o.updated_at=nowISO();persist()}
    updateOrderPaymentButtons();try{await navigator.clipboard.writeText(d.url)}catch{}toast(`Stripe ${kind==="deposit"?"deposit":"payment"} link created + copied`);
  }catch(e){toast(e?.message||"Could not create Stripe link")}finally{delete btn.dataset.busy;btn.disabled=false;btn.textContent=old;updateOrderPaymentButtons()}
}
async function copyOrderPaymentLink(){const link=$("orderPaymentLink").value.trim();if(!link)return toast("Create a payment link first");try{await navigator.clipboard.writeText(link);toast("Payment link copied")}catch{toast("Couldn't copy the link")}}
async function markOrderPaid(){if(!editingOrderId)return;const quote=Math.max(0,Number($("orderPrice").value||0));if(quote<=0)return toast("Set the order price first");$("orderPaymentStatus").value="paid";$("orderPaymentAmount").value=quote;if(!$("orderPaymentMethod").value)$("orderPaymentMethod").value="Cash";updateOrderPaymentButtons();toast("Marked paid — save the order to confirm") }
...
function renderCustomerPortalOrder(o){
  ...
  const notes=rawNotes.filter(n=>!String(n).trim().toLowerCase().startsWith(PAYMENT_NOTE_PREFIX.toLowerCase()));
  $("customerPortalNotesWrap").classList.toggle("hidden",!notes.length);
  $("customerPortalNotes").innerHTML=notes.map(n=>`<p>${safe(n)}</p>`).join("");
  activeCustomerPortalPaymentLink="";
