...
function updateOrderEditorSummary(){
  ...
  if(paymentSummary){
    const statusLabel=({unpaid:"Unpaid",deposit_paid:"Deposit paid",paid:"Paid",refunded:"Refunded"})[$("orderPaymentStatus")?.value]||"Unpaid";
    const paid=Math.max(0,Number($("orderPaymentAmount")?.value||0));
    const method=$("orderPaymentMethod")?.value||"";
    const parts=[statusLabel];
    if(paid>0)parts.push(money(paid));
    if(method)parts.push(method);
    paymentSummary.textContent=parts.join(" · ");
  }
  const notesSummary=$("orderNotesSummary");
  if(notesSummary){
    const notes=String($("orderNotes")?.value||"").trim();
    notesSummary.textContent=notes ? (notes.length>48?notes.slice(0,48)+"…":notes) : "Colors, delivery details, anything extra";
  }
}
