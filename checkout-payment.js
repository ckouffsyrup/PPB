/* PrintBook 5.19.4 — direct checkout, no quote emails/actions, ready email only. */
(() => {
  ...
  // Direct checkout workflow: Requested -> Approved -> Printing -> Ready -> Completed.
  // No Quoted/Accepted states and no quote email action anywhere in the admin UI.
  ...
  function stripQuoteAdminUI(){
    ...
    document.querySelectorAll('.order-card-actions button').forEach(btn=>{const t=btn.textContent.trim().toLowerCase();if(t.includes('quote'))btn.remove()});
  ...
  const originalRenderOrders=renderOrders;renderOrders=function(){originalRenderOrders();stripQuoteAdminUI()};
  stripQuoteAdminUI();
  ...
  `;document.head.appendChild(style);window.PRINTBOOK_BUILD='5.19.4';renderCustomerOrderCart();renderOrders();loadGlobalPaymentMethods().then(()=>{if($("customerCartDialog")?.open)ensureCheckoutPayment({loading:false})}).catch(()=>{});
})();
