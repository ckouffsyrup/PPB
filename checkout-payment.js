/* PrintBook 5.19.2 — global saved payment methods are the checkout source of truth. */
(() => {
  ...
  function hasPaymentMethodData(raw){
    return !!raw&&typeof raw==="object"&&!Array.isArray(raw)&&Object.values(raw).some(m=>m&&String(m.detail||"").trim());
  }

  function cacheGlobalPaymentMethods(raw){
    const methods=normalizedPaymentMethods(raw||{});
    globalPaymentMethods=methods;
    storeAvailability={...storeAvailability,payment_methods:methods};
    // Admin devices keep the same cloud-backed values locally as a resilient fallback.
  ...
  async function loadGlobalPaymentMethods(force=false){
    if(globalPaymentLoadPromise&&!force)return globalPaymentLoadPromise;
    globalPaymentLoadPromise=(async()=>{
      try{
        const res=await fetch(`${PUBLIC_PAYMENT_METHODS_URL}?t=${Date.now()}`,{method:"GET",headers:{Accept:"application/json"},cache:"no-store"});
        let data={};try{data=await res.json()}catch{}
        if(!res.ok)throw new Error(data?.error||`Payment methods request failed (${res.status})`);
        return cacheGlobalPaymentMethods(data?.payment_methods||{});
      }catch(err){
        console.error("Global payment methods load failed",err);
        const fallback=storeAvailability?.payment_methods||settings?.paymentMethods||{};
        if(hasPaymentMethodData(fallback))return cacheGlobalPaymentMethods(fallback);
        globalPaymentMethods=normalizedPaymentMethods({});
        return globalPaymentMethods;
      }finally{
        globalPaymentLoadPromise=null;
      }
    })();
    return globalPaymentLoadPromise;
  }

  function paymentRows(){
    const source=globalPaymentMethods||storeAvailability?.payment_methods||settings?.paymentMethods||{};
    const methods=normalizedPaymentMethods(source);
    return Object.entries(methods)
      .filter(([,m])=>m?.enabled&&String(m.detail||"").trim())
      .map(([key,m])=>({label:PAYMENT_METHOD_DEFS?.[key]?.label||m.label||key,detail:String(m.detail||"").trim()}));
  }
  ...
  function ensureCheckoutPayment({loading=false}={}){
    ...
    const submit=$("submitCustomerCartBtn");if(submit){submit.textContent='Submit Order';submit.disabled=!rows.length&&loading;}
  ...
  async function refreshCheckoutPayment(){
    ensureCheckoutPayment({loading:true});
    await loadGlobalPaymentMethods(true);
    ensureCheckoutPayment({loading:false});
  }
  ...
  function ensurePaymentOrderNumberReminder(){
    ...
    if(numberEl){
      ...
      if(card){
        ...
        if(!reminder){reminder=document.createElement('div');reminder.id='customerPaymentOrderNumberReminder';reminder.className='customer-payment-order-number-reminder';card.insertAdjacentElement('afterend',reminder)}
        reminder.innerHTML=`<small>IMPORTANT FOR PAYMENT</small><strong>USE ORDER NUMBER <span>${safe(numberEl.textContent||'KP-0000')}</span> AS YOUR PAYMENT SUBJECT / NOTE</strong><p>This is how your payment gets matched to your order.</p>`;
      }
    }
    const portalNumber=$("customerPortalNumber");
    const paymentBox=$("customerPortalPaymentBox");
    if(portalNumber&&paymentBox){
      let reminder=$("customerPortalPaymentOrderNumberReminder");
      if(!reminder){reminder=document.createElement('div');reminder.id='customerPortalPaymentOrderNumberReminder';reminder.className='customer-payment-order-number-reminder portal-reminder';paymentBox.insertAdjacentElement('beforebegin',reminder)}
      reminder.innerHTML=`<small>WHEN YOU PAY</small><strong>USE ORDER NUMBER <span>${safe(portalNumber.textContent||'—')}</span> AS YOUR PAYMENT SUBJECT / NOTE</strong><p>Please include it exactly as shown so your payment can be matched to this order.</p>`;
    }
  }

  const originalRender=renderCustomerOrderCart;
  renderCustomerOrderCart=function(){originalRender();ensureCheckoutPayment({loading:!globalPaymentMethods})};
  const originalOpen=openCustomerOrderCart;
  openCustomerOrderCart=function(){const r=originalOpen();refreshCheckoutPayment().catch(()=>ensureCheckoutPayment({loading:false}));return r};

  const confirmDialog=$("customerOrderConfirmationDialog");
  if(confirmDialog)confirmDialog.addEventListener('toggle',()=>{if(confirmDialog.open)setTimeout(ensurePaymentOrderNumberReminder,0)});
  const portalDialog=$("customerOrderPortalDialog");
  if(portalDialog)portalDialog.addEventListener('toggle',()=>{if(portalDialog.open)setTimeout(ensurePaymentOrderNumberReminder,0)});
  const observer=new MutationObserver(()=>{if(confirmDialog?.open||portalDialog?.open)ensurePaymentOrderNumberReminder()});
  if(confirmDialog)observer.observe(confirmDialog,{subtree:true,characterData:true,childList:true});
  if(portalDialog)observer.observe(portalDialog,{subtree:true,characterData:true,childList:true});
  ...
  window.PRINTBOOK_BUILD='5.19.2';
  renderCustomerOrderCart();
  loadGlobalPaymentMethods().then(()=>{if($("customerCartDialog")?.open)ensureCheckoutPayment({loading:false})}).catch(()=>{});
})();
