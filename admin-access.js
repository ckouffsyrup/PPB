/* PrintBook 5.23.1 — owner-only admin access + remembered mobile admin device. */
(() => {
  ...
  // If iOS/Home Screen stripped ?admin=1, restore the visible URL without reloading.
  ...
  function forgetMobileAdmin(){
    try{localStorage.removeItem(MOBILE_ADMIN_PREF)}catch{}
  }

  function openCustomerStoreFromAdmin(){
    forgetMobileAdmin();
  ...
  function ensureMobileStoreExit(){
    ...
    wrap.id="mobileAdminStoreExit";
    wrap.className="mobile-admin-store-exit";
    const btn=document.createElement("button");
    btn.type="button";
    btn.textContent="View Customer Store";
    btn.onclick=openCustomerStoreFromAdmin;
  ...
  async function unlockAdmin(user){
    ...
    rememberThisMobileAsAdmin();
    removeGate();
    ensureMobileStoreExit();
  ...
  // The normal storefront does not expose an owner-login path unless this mobile device
  // was explicitly remembered by a successful owner login.
  if(!adminRoute){
